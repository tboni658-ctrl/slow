...
function renderMainInterface() {
    ...
    positions.forEach((pos, index) => {
        ...
        // Disini ambil dari displayed wallet (yang sudah ditambah daily pnl) agar rasio margin akurat
        // Tapi kita perlu hitung dulu diluar loop, jadi sementara ambil dari input + pnl
        // Untuk penyederhanaan di kartu, kita pakai base calculation
...
function updateGlobalTotals() {
    // 1. Ambil Input (Saldo Awal & Persen)
    const initialWallet = parseFloat(document.getElementById('in-wallet').value) || 0;
    ...
    // 2. Hitung Nominal PnL Harian (USDT)
    const dailyUsdt = initialWallet * (dailyPercent / 100);
    
    // 3. Hitung Saldo Dompet SEKARANG (Saldo Awal + PnL Harian)
    const currentWalletBalance = initialWallet + dailyUsdt;

    // --- Format Teks PnL Harian ---
    ...
    const percentStr = absPercent.toFixed(2); // Pakai titik
    ...
    // --- Hitung Total PnL Posisi (Unrealized) ---
    let totalUnrealizedPNL = 0;
    
    // Kita loop juga untuk update Risk Ratio karena butuh Saldo Akhir
    positions.forEach((pos, index) => {
        let pnl = 0;
        if(pos.entry > 0) {
            if(pos.side === 'SHORT') {
        ...
        // Update Risk Ratio per posisi (Margin / (Saldo Akhir + PnL))
        const margin = pos.size / pos.leverage;
        const equity = currentWalletBalance + pnl;
        let risk = 0;
        if (equity > 0) {
            risk = (margin / equity) * 10; // Logika sederhana
        }
    ...
    // 4. Hitung Saldo Margin (Saldo Akhir + Unrealized PNL)
    const totalMarginBalance = currentWalletBalance + totalUnrealizedPNL;

    // --- Update Tampilan ---
    
    // Tampilkan Saldo Dompet (Hasil Perhitungan)
    ...
    // Tampilkan PNL Belum Terealisasi
    ...
    // Tampilkan Saldo Margin (Paling Atas)
